...
| 測試案例 | 資料量 $n$ | $n$預期最快演算法 | 實際執行時間 (us)|記憶體使用量 (KB)|
|----------|--------------|----------|----------|----------|
| 測試一   | $n = 500$      | Quick Sort       | 90       | 406       |
| 測試二   | $n = 1000$     | Quick Sort       | 178       | 1629        |
| 測試三   | $n = 3000$      | Heap Sort       | 539        | 14680        |
| 測試四   | $n = 5000$      | Composite Sort     | 887       | 39863        |

### 編譯與執行指令

```shell
$g++ main.cpp -lpsapi -o sort_test [cite: 125]$ ./sort_test
```

### 結論

1. 程式能正確執行五種排序演算法，並與測試結果比對正確。  
2. 實驗證實 $O(n \log n)$ 演算法在大數據量下遠優於 $O(n^2)$。  
3. 透過 Composite Sort 成功對應不同輸入資料，在最短時間內完成排序。
